...
import { createGiteeReleasesProvider } from './update/providers/gitee';
import { createGitlabReleasesProvider } from './update/providers/gitlab';
...
async function registerIpcHandlers(): Promise<void> {
      ...
      if (orchEventsMatch) {
        ...
        relayOrchEventsStream(
          ...
          if (e instanceof Error && e.name !== 'AbortError') {
            ...
            // 2026-09 修复: relay 失败必须通知渲染端, 否则编排订阅永久卡
            // connecting(渲染端监听 {event}-error 通道终结生成器)。
  ...
  if (!updateManager) {
    ...
    providerRegistry.register('gitlab', (cfg) =>
      createGitlabReleasesProvider({
        id: cfg.id,
        displayName: cfg.displayName,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        config: cfg.config as any,
...
app
  ...
  .then(async () => {
    ...
    try {
      backendProc = spawnBackend();
      logger.info('main: backend spawn returned successfully', {
        pid: backendProc?.pid,
      ...
      logger.error('main: backend spawn threw exception', {
        error: err instanceof Error ? err.message : String(err),
      });
