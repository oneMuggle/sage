  ...
  return (
    <div className="p-3 border border-border rounded-radius-sm bg-surface cursor-pointer hover:border-primary transition-colors">
      <div className="flex items-center justify-between mb-1">
        ...
        <span className={`text-[11px] px-2 py-0.5 rounded font-mono ${tagClass}`}>{tagLabel}</span>
      </div>
      <div className="text-xs text-muted leading-relaxed">
        {content.length > 80 ? content.substring(0, 80) + '...' : content}
      </div>
      <div className="text-[11px] text-muted mt-1.5 font-mono">
        创建于 {formatDate(memory.created_at)} · 引用 {memory.access_count || 0} 次 · 置信度{' '}
        {(memory.importance / 10).toFixed(2)}
      </div>
    </div>
  );
}
