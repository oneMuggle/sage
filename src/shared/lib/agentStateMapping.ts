...
export function agentStateToText(state: AgentState, toolName?: string): string | null {
  switch (state) {
    ...
    case 'reasoning_final': // 2026-09-02: 全量对齐事件,不渲染占位文本(同 reasoning_delta)
    case 'content_delta':
    case 'idle':
    case 'done':
    ...
    case 'context_pressure': // TM2 (DSH 对标 R11): 水位写 store 供徽章渲染,不进气泡占位
...
export function agentStateToPhase(state: AgentState | null | undefined): PhaseDisplay | null {
  ...
  switch (state) {
    ...
    case 'reasoning':
    case 'reasoning_delta':
    case 'reasoning_final': // 2026-09-02: 全量对齐事件,显示同 reasoning_delta(Brain + 推理中)
    ...
    case 'context_pressure': // TM2 (DSH 对标 R11): 水位徽章自渲染,无独立 UI 阶段
