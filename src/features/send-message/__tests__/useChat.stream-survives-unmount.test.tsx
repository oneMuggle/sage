...
// S2: 槽位按会话键控,测试统一走固定会话 id
const SESS = 'sess-unmount-1';
...
describe('useChat — 流式状态跨组件实例存活', () => {
  ...
  it('Chat 页卸载再挂载,流式状态从 store 恢复', () => {
    ...
    useChatStreamStore.getState().startStream(SESS, 'msg-A', { initialContent: '🤔 思考中…' });
    useChatStreamStore.getState().appendContent(SESS, 'msg-A', '你好,');
    useChatStreamStore.getState().replaceContent(SESS, 'msg-A', '⚙ 调用工具…');

    // 2. 模拟 Chat 页卸载 —— 这在 store 化前会丢失 streaming state
    //    (此处只能断言: store 的内容不被任何 hook 卸载影响)
    expect(selectSessionSlots(useChatStreamStore.getState(), SESS).streaming?.messageId).toBe(
      'msg-A',
    );

    // 3. 后端又推了一个 content_delta('继续工作')
    useChatStreamStore.getState().appendContent(SESS, 'msg-A', '继续工作');

    // 4. 用户切回 Chat 页 —— 新 hook 实例 selector 同一 store
    //    (这里我们只断言 store 仍是累计后的状态,而不去实际 render 第二实例
    //     避免引入 useSettings 的额外 mock 开销)
    const s = selectSessionSlots(useChatStreamStore.getState(), SESS);
