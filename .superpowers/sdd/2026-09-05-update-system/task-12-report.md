...
Verification:
- `npm run test:run -- electron/tests/updateIntegration.test.ts` — passed, 14/14 tests.
- `npm run test:run -- electron/tests/update*.test.ts` — passed, 6 files and 88/88 tests.
- Direct TypeScript check for the new test (`npx tsc --noEmit --target es2022 --module esnext --moduleResolution bundler --skipLibCheck --types vitest/globals electron/tests/updateIntegration.test.ts`) — passed.
- `npm exec eslint electron/tests/updateIntegration.test.ts` — passed.
