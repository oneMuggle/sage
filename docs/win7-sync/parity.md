...
- 异动文件：**1937 → 865 (B1) → 832 (B2) → 235 (B3) → 226 (B4-B6)**（`git diff --name-only origin/main HEAD`）
- B4-B6 后剩余 226 文件：全部为 win7 必要差异（py38 typing 回写 ~90、compat 层、win7 专属测试 58、docs 33、Memory 3-tab、D 类），**残差审计闭环**
...
| B4 | Orchestration（残差审计） | ✅ | 8cb42cbe | 残差仅 py38 typing 回写 / field_validator shim，无业务差异 |
| B5 | Web-access（mcp/web_access/llm_proxy 残差审计） | ✅ | 8cb42cbe | 同上；llm_proxy local-auth 3 用例本机失败已在 origin/main 复现（上游问题，非回归） |
| B6 | UI/electron 清理 | ✅ | 8cb42cbe | 删 win7 死代码（schemas/common、types/api.ts、SessionTreeView、eventsource.d.ts）、providers/index 对齐 main、agent.py 缩进；py38 0 / tsc 0×2 / vitest 57 pass / pytest 定向 826 pass |
...
1. 整体 merge 之后，剩余批次改为**残差审计**：`git diff HEAD origin/main -- <域>`，过滤 typing 回写行
   （`Optional[`/`List[`/`| None`/`from typing`）后看剩余 hunk，为零即"必要差异"直接记账。
2. win7 独有文件（`git diff --name-status` 的 `D` 行）逐个 `git grep` importer；无引用即删，避免长期漂移。
3. 本机失败先到 origin/main 检出上复现，再决定是否算回归（llm_proxy local-auth 属上游问题）。
4. 残差 226 文件是稳态：以后 main 合入后 `check_py38_compat.py` + `parity_report.py` 即可发现新增偏差。
