# 41. sage doctor — 安装/环境级 self-check CLI

> **最后更新**: 2026-08-07
> **适用版本**: Sage main @ feat/sage-doctor
> **背景计划**: [`../plans/2026-08-07_sage-doctor.md`](../plans/2026-08-07_sage-doctor.md)
> **关联章节**: [39 记忆系统与用户画像](./39-memory-user-profile.md)、[20 Electron 21 桌面壳](./20-electron.md)、[29 Electron 桌面日志](./29-electron-logging.md)
> **互补文档**: [`../user-manual/11-sage-doctor.md`](../user-manual/11-sage-doctor.md)（用户视角）；[`../user-manual/06-diagnostics.md`](../user-manual/06-diagnostics.md)（运行时 UI 诊断面板，本章是其 CLI 前置对应物）
...
### 41.3.1 `conda_env` — 当前 Python 是否在 Sage conda 环境
...
### 41.3.2 `backend_health` — FastAPI /health 端点连通性
...
### 41.3.3 `sqlite_writable` — `SAGE_USER_DATA_DIR` 目录可写性
...
### 41.3.4 `config_integrity` — `~/.sage/config/*.json` JSON 合法 + 必填字段
...
### 41.3.5 `port_backend` — 8765 端口占用（孤儿 backend）
...
### 41.3.6 `port_frontend` — 1420 端口占用（Vite dev server）
...
### 41.3.9 `llm_config` — `preferences.app_settings` 中至少 1 个 LLM endpoint 配置了 `apiKey`
...
### 41.3.10 `mcp_servers` — `$SAGE_USER_DATA_DIR/mcp_servers.json` 中每个 enabled server 的 `command` 可解析
...
### 41.3.11 `heavy_deps` — `hnswlib` / `jieba` / `sqlite_vec` 三个重依赖可 import
...
### 41.3.12 `log_dir_size` — `backend/logs/` 目录累计大小（防止日志膨胀）
...
### 41.5.2 启动流程注入：`electron/main.ts`
...
4. 对照 [§41.3 13 项检查详解](#413-13-项检查详解) 找到 root cause
...
`release/win7` cherry-pick 时**不需要**额外适配。

---

## 41.8 与 `docs/user-manual/06-diagnostics.md` 的关系
...
| `electron/main.ts` line 815-834 | 启动流程注入点 |
