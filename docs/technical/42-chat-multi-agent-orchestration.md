...
编排走既有 `/chat/stream` NDJSON 通道，新增 2 个中间态事件：
...
- multi：注册工具 + 计划块注入 system prompt + **先推 `task_plan`**
- `run_id = "orch-{uuid4()}"`
...
| 组件 | 职责 |
|---|---|
| `llmStream.ts` + `types.ts` | `AgentState` 增 `task_plan`/`task_status`；`TaskPlanItem`/`TaskPlanEvent`/`TaskStatusValue`/`TaskStatusEvent` 四窄类型；`AgentEvent` 6 个松散字段（`run_id`/`plan`/`task_id`/`status`/`goal`/`output_preview`）；`ChatConfig.orchestrationMode`。**双声明模式**：两文件需保持同步（useChat 经 `shared/api/index.ts` → `types.ts`） |
| `chatApi.chatStream` | invoke payload 增 `orchestrationMode: config?.orchestrationMode ?? null`（undefined → null → 后端默认 auto） |
| `useChat` | `sendMessage(content, sessionId, officeRefs, orchestrationMode)` 第 4 参透传；`TaskBoard` state（`{runId, plan, statuses}`）：`task_plan` 初始化板、`task_status` 按 `run_id` 匹配才合并（旧 run 忽略）、新消息清空 |
| `TaskTreeSection` | 任务树：每子任务一行 = 状态图标（queued ○ / running ◐ / done ✓ / failed ✗）+ agent_id 徽标 + goal；done/failed 可展开看 `output_preview`/`error`；顶部"子任务 x/y 完成" |
| `ProgressSection` / `RightPanel` | `taskBoard` prop 非空时渲染 TaskTreeSection，否则回落既有 tool-call 列表（简单任务零视觉噪音） |
