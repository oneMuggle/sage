...
4. 其他会话的消息不混入当前消息列表；
5. 合并后的列表保持时间顺序，避免切回页面时消息跳到错误位置。
...
- [ ] 为 `loadMessages` 增加“后端历史消息 + 未持久化本地消息”的回归测试
- [ ] 实现按消息 ID 的不可变合并逻辑
- [ ] 增加页面卸载/重新加载后流式回复仍能写回的测试覆盖
- [ ] 运行相关 Vitest 测试、TypeScript 检查和前端构建
- [ ] 运行代码审查并根据结果修正

## 风险评估与依赖

- 风险：后端消息时间戳与本地乐观消息时间戳不同，简单拼接可能改变顺序。实施时必须使用稳定的时间排序，并保留相同时间戳下的原始相对顺序。
