...
- [ ] `domain/` 零外部依赖（用 `import-linter` 验证）
- [ ] `ports/` 6 个接口已定义
- [ ] 至少 1 个端口有 ≥ 2 个 adapter 实现（生产 + 测试 mock）
- [ ] 端到端对话 + 工具调用在生产代码下行为不变
- [ ] `docs/technical/02-architecture.md` 更新，新增 `18-hexagonal.md`
...
| 指标                | 基线（2026-06-05 P0-T3 实测）                   | P1 末实测                            | 目标                            | 衡量方式                        |
| ------------------- | ----------------------------------------------- | ------------------------------------ | ------------------------------- | ------------------------------- |
| 后端测试覆盖率      | **43%**（2895 stmts, 1650 missed；41 测试全过） | **84%**（2874 stmts, 862 missed）    | ≥ 80%                           | `pytest --cov`                  |
| 后端测试数          | 41                                              | **383**                              | ≥ 80（P1 末 +PG1.1–PG1.6 补齐） | `pytest --collect-only`         |
| 前端测试覆盖率      | TBD（P0-T8 末测）                               | **features 87.72%, entities 91.18%** | ≥ 60%（核心 ≥ 80%）             | `vitest --coverage`             |
| 前端测试数          | 17 (P0) → 53 (P1)                               | **53** (P1 末实测 PG1.14)            | ≥ 80                            | `vitest run`                    |
| CI 跑通时间         | 无 CI                                           | ≤ 8 分钟（无 Tauri）                 | ≤ 8 分钟（无 Tauri）            | GH Actions                      |
| 端到端可用率        | 单平台                                          | Web + 桌面三平台 + Win7              | Web + 桌面三平台 + Win7         | 手动 + E2E                      |
| a11y 评分           | TBD（P3 末测）                                  | 未测                                 | ≥ 95                            | Lighthouse                      |
| 文件 < 800 行       | 100%                                            | 100% 维持                            | 100% 维持                       | `wc -l` 扫描                    |
| 模块依赖图          | 散乱                                            | 后端六边形 + 前端 FSD 清晰           | 后端六边形 + 前端 FSD 清晰      | import-linter / eslint 边界规则 |
| 平均 PR review 时间 | 无基线                                          | 无基线                               | ≤ 24h                           | GH 统计                         |
