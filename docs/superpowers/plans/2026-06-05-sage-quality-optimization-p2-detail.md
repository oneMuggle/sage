...
- [x] `backend/domain/` 零外部依赖（仅 typing + dataclass + enum + abc）
- [x] `backend/ports/` 6 个 Protocol 接口定义完成
- [x] `backend/adapters/` 至少 1 个生产 + 1 个 mock 实现覆盖每个 port
- [x] `backend/application/services/chat_service.py` 编排 6 个 ports
- [x] `backend/api/routes.py` 改写为 in-adapter（仅 HTTP 序列化 + 调用 service）
- [x] `import-linter` 强制 5 层依赖图
- [x] **双轨保留**：旧路径 `core/agent.py` 等保留为 `core/legacy/`，通过 `API_MODE=legacy` 切换
- [x] 端到端 383 个测试仍全过
- [x] 整体覆盖率 ≥ 80%（不下降）
- [x] `docs/technical/18-hexagonal.md` 发布
- [x] `docs/technical/02-architecture.md` 重写为六边形视角
...
- [x] Read `core/agent_state.py`（81 行 — 状态枚举）
- [x] Read `core/errors.py`（46 行 — LLMError / LLMErrorType）
- [x] Read `core/exceptions.py`（200 行 — 异常体系）
- [x] Read `core/agent.py` 找出 dataclass（消息、决策等纯数据类）
- [x] 识别应该迁移的纯模型 vs 应该留在 application 层的服务逻辑
...
- [x] Read `/home/fz/project/sage/backend/core/llm_client.py` 全文
...
- [x] Read `backend/data/database.py`、`backend/memory/*` 找到所有 sqlite 访问点
- [x] 列出 StoragePort 需要的操作
...
- [x] Read `/home/fz/project/sage/backend/tools/registry.py`
...
- [x] Read `/home/fz/project/sage/backend/api/routes.py`
...
### P2 阶段
- [x] PG2.1: domain/ 抽离（5 文件）
- [x] PG2.2: ports/ 接口（6 Protocol）
- [x] PG2.3: httpx LLM adapter
- [x] PG2.4: mock LLM adapter
- [x] PG2.5: storage adapters (sqlite + memory)
- [x] PG2.6: tool inproc adapter
- [x] PG2.7: metric adapters (骨架)
- [x] PG2.8: event adapters (骨架)
- [x] PG2.9: ChatService
- [x] PG2.10: 重写 api/routes.py 为双轨
- [x] PG2.11: import-linter 5 层依赖图
- [x] PG2.12: 端到端回归（hex + legacy）
- [x] PG2.13: 旧 core/ 移入 legacy/
- [x] PG2.14: 文档更新（18-hexagonal + 02 重写）
