...
if __name__ == "__main__":
    # Test workspace scope manager
    import tempfile

    print("Testing WorkspaceScopeManager...")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        manager = WorkspaceScopeManager(db_path=db_path)

        print("\n1. Testing get_workspace_config...")
        config1 = manager.get_workspace_config(workspace1)
        assert config1.workspace_path == str(workspace1)
        assert config1.auto_discover is True
        print("✅ Config loaded")

        print("\n2. Testing set_server_enabled...")
        manager.set_server_enabled(workspace1, "server-1", True)
        manager.set_server_enabled(workspace1, "server-2", False)

        config1 = manager.get_workspace_config(workspace1)
        assert "server-1" in config1.enabled_servers
        assert "server-2" in config1.disabled_servers
        print("✅ Server enabled/disabled")

        print("\n3. Testing add_custom_server...")
        custom_server = DiscoveredMcpServer(
            name="custom-server",
            source="workspace",
            command="custom-cmd",
            description="Custom server",
        )
        manager.add_custom_server(workspace1, custom_server)

        config1 = manager.get_workspace_config(workspace1)
        assert len(config1.custom_servers) == 1
        assert config1.custom_servers[0].name == "custom-server"
        assert "custom-server" in config1.enabled_servers
        print("✅ Custom server added")

        print("\n4. Testing multiple workspaces...")
        config2 = manager.get_workspace_config(workspace2)
        assert config2.workspace_path == str(workspace2)
        assert len(config2.enabled_servers) == 0  # Different workspace
        print("✅ Workspace isolation works")

        print("\n5. Testing list_workspaces...")
        workspaces = manager.list_workspaces()
        assert str(workspace1) in workspaces
        assert str(workspace2) in workspaces
        print(f"✅ Found {len(workspaces)} workspace(s)")

        print("\n6. Testing delete_workspace...")
        manager.delete_workspace(workspace2)
        workspaces = manager.list_workspaces()
        assert str(workspace2) not in workspaces
        print("✅ Workspace deleted")

    print("\n✅ All workspace scope tests passed!")
