    ...
    *,
    ...
    for page in pdf:
        ...
        if page_paragraphs == 0 and not page_tables and page_images > 0:
            # Round D P10: 扫描页回退 —— 复用 office-p4a 的 pytesseract OCR
            # 兜底（SAGE_OCR=1 opt-in + 依赖探测都在 ocr_page_if_needed 内）。
            # text 传空串：本分支已知该页无文本层。
            from . import ocr as _ocr  # call-time import — optional heavy dep

            ocr_text = _ocr.ocr_page_if_needed(page, "")
...
def _convert_inner(source: Path, workspace: Path, out_filename: str) -> PdfToWordResult:
    ...
    ocr_ok, _ocr_reason = _ocr.ocr_available()
    return PdfToWordResult(
        ok=True,
        ...
        ocr_available=ocr_ok and _ocr.is_ocr_enabled(),
    )
