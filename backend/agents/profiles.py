...
def create_default_agents() -> List[AgentProfile]:
    ...
    return [
        ...
        AgentProfile(
            ...
            # 2026-09-03 (win7 cherry-pick of PR #402): 保留 memory_save。
            # main PR #402 删除它，但 win7 上 memory_tool.py 仍有 memory_save 实现，
            # win7 单测 test_researcher_profile_has_memory_tools 强约束 researcher 必须含
            # memory_save（researcher 持久化研究记录的工作流设计）。只 cherry-pick
            # 主线公认的 bug fix 部分（coder 工具名错位），跳过与 win7 设计冲突的变更。
