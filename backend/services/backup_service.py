...
def get_backup_dir(db_path: Optional[str] = None) -> Path:
...
def create_backup(reason: str = "manual", db_path: Optional[str] = None) -> Optional[Dict[str, Any]]:
...
def restore_backup(name: str, db_path: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """安排恢复指定备份（下次启动生效）。
...
def apply_pending_restore(db_path: Optional[str] = None) -> bool:
