...
- 解析: 复用 REST 路由（``/api/v1/skills*``）持有的 ``InprocSkillAdapter``
  单例（``backend.api.legacy_routes._get_skill_adapter``）——同一个
  SkillRegistry、同一份 enabled 状态。未知技能 → 错误 ToolResult 并列出
  所有可用技能名。
