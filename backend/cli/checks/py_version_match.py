...
class PyVersionMatchCheck:
    ...
    description = "Python 版本与 backend/requirements-py38.txt 一致性"

    def run(self) -> CheckResult:
        # backend/cli/checks/py_version_match.py -> ../../../backend/
        # win7 LTS: python 约束优先从 -py38 变体文件读取（requirements-py38.txt 不存在时回退
        # requirements.txt）。main 分支始终读 requirements.txt。
        backend_dir = Path(__file__).resolve().parents[2]
        req_path = backend_dir / "requirements-py38.txt"
        if not req_path.exists():
        ...
        if parsed is None:
            return CheckResult(
                self.name,
                Severity.INFO,
                f"backend/{req_name} 未声明 python 版本约束",
            )
        ...
        if _compare(current, op, target):
            return CheckResult(
                self.name,
                Severity.INFO,
                f"Python {current[0]}.{current[1]} 满足 {req_name} 约束 python{op}{target_str}",
            )

        return CheckResult(
            self.name,
            Severity.CRITICAL,
            f"Python {current[0]}.{current[1]} 不满足 {req_name} 约束 python{op}{target_str}",
            "切到正确的 conda 环境",
        )
