...
def _projects_registry_paths() -> List[Path]:
    """P12 对齐（main P6）：projects 注册表（backend/data/project_repo.py）
    中的路径。
...
def authorize_registered_project(project_path: str) -> Path:
    ...
    # P12 对齐（main P6）：projects 注册表命中同样视为已登记
    # （recents ∪ registry）。与 recents 分支同款 try/except ——
    # 任何异常一律视为未登记（fail-closed）。
    if not registered:
        try:
