...
- 启动 lifespan 时若 agents 表空, 种子化 5 个默认 agent (primary / researcher /
  coder / memory_manager / writer), 见 backend/agents/profiles.py:create_default_agents()
- GET /agents → 200 + 5 个 agent profile
...
EXPECTED_DEFAULT_IDS = {"primary", "researcher", "coder", "memory_manager", "writer"}


@pytest.mark.asyncio()
async def test_list_agents_returns_four_defaults_after_lifespan(client):
    """lifespan 启动后 GET /agents 必返回 5 个默认 agent."""
    ...
    assert ids == EXPECTED_DEFAULT_IDS, f"期望默认 5 个 agent, 实际 {ids}"
