...
async def test_health_latency_under_concurrent_chat_stream_create(client, mock_llm_ok):
    """PR B §1.2 修复回归:N 并发 /api/v1/chat/stream 期间,/health P99 < 100ms。
    ...
    ## Tail probe 设计 (与 session CRUD 测试不同)

    chat_stream_create handler 只做轻量 office auth 后立即返回 200,
    真正的 SQLite 写 (agent.run_loop 的消息持久化) 发生在 handler
    返回后由 ``registry.create`` 启动的后台 producer task 里
    (``backend/api/chat_stream_registry.py:110-112``)。
    因此 ``await asyncio.gather(*write_tasks)`` 收到响应就停 probe
    → probe 来不及采到 producer 的 SQLite 阻塞,样本数严重不足
    (< 20)。

    解决:gather 完之后**继续** probe 3 秒 (tail probe),让后台
    producer 有时间跑完它的 SQLite 写,被 /health 探针捕获到真实
    阻塞。这是测试本身可达性所需的最小变更,不破坏 brief 核心意图
    (暴露 chat handler 阻塞)。session CRUD 测试不需要 tail probe,
    因为其 SQLite 写全部在 handler 主体内完成,gather 完成时阻塞已结束。
    """
    ...
    try:
        ...
        # tail probe: 后台 producer 的 SQLite 写发生在 handler 返回 200 之后,
        # gather 完不能立即停 probe。给 3s 让 producer 工作,采集真实阻塞。
        tail_seconds = 3.0
        tail_end = time.perf_counter() + tail_seconds
    ...
    assert len(health_samples) >= 30, (
        f"健康探针样本不足: {len(health_samples)} < 30 "
        f"(handler 期间 ~15 + tail 3s 应能再采 ~1500)"
    )
