...
# 本测试维护一份"必须 keep_async"的精确白名单(10 个,L599/L641/734/917/1031/1342/1594/1602/1617/1628)。
# 所有其他 async def 必须有 await,否则应降级为 def。
# NOTE (win7 sync #294): main 的白名单含 compact_session(会话压缩,win7 未移植);
...
def test_async_handler_count_matches_design():
    """legacy_routes.py 应有 10 个 async def handler(PR #294 §1.2 修复后 + win7 独有 4 个 memory)。"""
    src_path = LEGACY_ROUTES_PATH
    src = src_path.read_text(encoding="utf-8")
    funcs = _load_top_level_functions(src)
    async_endpoints = [
    ...
    # 修复后:10 个 keep_async (execute_skill, execute_slash_command, import_skills,
