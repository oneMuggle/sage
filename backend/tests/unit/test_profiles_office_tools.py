...
def test_primary_sees_all_office_tools(registry, bound_ctx):
    ...
    # office_analyze_ppt_template / office_fill_ppt_template, 21 → 23 件。
    # （win7 分支未同步 Round 39 office_refresh_toc，计数与 main 不同）
    assert visible == [
        "office_analyze",
...
def test_writer_sees_read_write_but_not_delete(registry, bound_ctx):
    ...
    # office_fill_ppt_template, 20 → 22 件（仍不给 office_delete）。
    assert visible == [
        "office_analyze",
        "office_analyze_ppt_template",
        "office_analyze_word_template",
...
def test_list_and_read_hidden_without_workspace_binding(registry):
    ...
    False(它们支持 file_path 模式), 所以仍可见 —— 这是有意的。2026-09
