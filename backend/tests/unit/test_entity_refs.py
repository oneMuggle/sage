...
class TestResolvers:
    ...
    def test_process_end_to_end_with_failures(self):
        ...
        with patch("backend.memory.get_memory_manager", side_effect=RuntimeError):  # noqa: SIM117 — py38 不支持括号多上下文 with
            with patch.object(entity_refs, "_wiki_project_root", return_value=None):  # noqa: SIM117 — py38 不支持括号多上下文 with
